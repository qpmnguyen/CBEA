...
## Loading sample data  
First, we can load some pre-packaged data sets contained in `CBEA`. Here we're loading the `phyloseq` version of the same data set. The `CBEA` package also supports `TreeSummarizedExperiment` objects, where users can perform the conversion using the `mia` package.          
```{r load_data}
data("hmp_gingival")
...
* `r Biocpkg("CBEA")` `r Citep(bib[["CBEA"]])`
