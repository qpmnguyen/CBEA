...
#' @describeIn const_set Convert taxonomic tables from \code{\linkS4class{phyloseq}} to \code{\linkS4class{BiocSet}}
#'     from a certain rank.
...
#' @describeIn const_set Convert taxonomic tables from \code{\linkS4class{TreeSummarizedExperiment}} to \code{\linkS4class{BiocSet}}
#'     from a certain rank.
...
#'     This matrix is a matrix of 1 or 0s indicating membership of taxa (as row or column names)
#'     with relevant sets (as row or column names)
#' @param sets_are_rows (Logical). Indicate whether sets are rows or as columns. Affect set names
...
setMethod("const_set", "matrix", function(obj, sets_are_rows = FALSE) {
  if (!all(obj %in% c(0, 1))) {
    stop("Please convert element-set membership matrix to
             data.frame and specify id and member columns. See ?const_set for details")
  }
...
#' @describeIn const_set Convert \code{data.frame} element set or \code{tibble} element set
#'     to \code{BiocSet}. We expect at least a 2-column data frame with the taxa names
...
setMethod("const_set", "data.frame", function(obj, id, member) {
  ...
  if (all(obj %in% c(0, 1))) {
    warning("This looks like a logical dummy variable matrix indicating membership. Dispatching to the matrix signature")
    obj <- as.matrix(obj)
    sets <- const_set(obj)
  }
  set_names <- obj %>%
...
#'     not in the taxa_names of the data container. Throws an error if there are no elements in BiocSet that matches
#'     taxa names of the data container.
#' @export
#' @param obj Data container object containing taxonomic relative abundance data with taxa and sample names
...
#' @describeIn unify_sets Unify set and data container for \code{\linkS4class{TreeSummarizedExperiment}}
...
#' @describeIn unify_sets Unify set and data container for \code{\linkS4class{phyloseq}}
...
#' @param taxa_are_rows (Logical). Indicate whether taxa are stored as rows or columns of the data.frame/matrix.
#' @param tax_ids (String). Indicate column name with all the names of taxa. Can be "rownames" if
#'     taxa names are stored as row names. Only used if \code{taxa_are_rows = TRUE}.
#' @importFrom magrittr %>%
#' @importFrom dplyr pull
#' @importFrom rlang sym
#' @export
setMethod("unify_sets", "data.frame", function(obj, set, tax_ids = NULL, taxa_are_rows = TRUE) {
  element <- NULL # R CMD CHECK
...
#' @describeIn unify_sets Unify set and data container for \code{\linkS4class{matrix}}
